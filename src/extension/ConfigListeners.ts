...
// export function upsertSoundCommand (newCue: SoundCommandChangeMessage, ack?: ListenForCb): void {
...
//   if ((newCue.commandName != null) && (ack != null) && !ack.handled) {
//     ack(Error('Cue command change failed: No cue command name was provided.'))
//     return
//   }
//   if (((newCue.mappedCues != null) || newCue.mappedCues.length === 0) && (ack != null) && !ack.handled) {
//     ack(Error(`Cue command change failed: No cues were mapped to command ${newCue?.commandName ?? 'undefined command'}.`))
//     return
//   }
...
//       allCuesAreValid: newCue.mappedCues.every((mc: string) => knownSoundCues.value.includes(mc)),
...
export function deleteSoundCommand (commandName: string, ack?: ListenForCb): void {
  if (nodecgHandle.current == null) {
  ...
  if ((commandName.length === 0) && (ack != null) && !ack.handled) {
